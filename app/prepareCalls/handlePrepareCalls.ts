...
import { Call, PrepareOrSendCallsParams } from "../types";
...
export type PrepareCallsRequest = {
  method: "wallet_prepareCalls";
  params: PrepareOrSendCallsParams;
};

export async function handlePrepareCalls(request: PrepareOrSendCallsParams) {
  if (
    !request[0].calls.every(
      (call) => call.chainId === request[0].calls[0].chainId,
    )
  ) {
  ...
  const { hash } = await getUserOpHash(userOpToSign);
    ...
    result: [
      {
        data: {
          ...
          values: deepHexlify({ ...userOpToSign, signature: "0x" }),
        },
        hash,
      },
    ],
